...
class SiteMapController extends Controller
{
    ...
    function Index()
    {
        ...
        $industrylinks=IndustryNew::where('ismake',1)->orderBy('id','desc')->get();
        ...
        for ($i=0;$i<count($newsurllinks);$i++)
        {
            $urlsets.="
<url>
    <loc>$appurl/{$newsurllinks[$i]->arctype->real_path}/{$newsurllinks[$i]->id}/</loc>
    ...
    <priority>1.0</priority>
        ...
        for ($i=0;$i<count($brandlinks);$i++)
        {
            $urlsets.="
<url>
    <loc>$appurl/ganxi_pinpai_/{$brandlinks[$i]->pinyin}/</loc>
    ...
    <priority>1.0</priority>
        ...
        $index=1;
        #------------------------------------------------------------------------------------------
        for ($i=0;$i<count($industrylinks);$i++)
        {
            $urlsets.="
<url>
    <loc>$appurl/{$industrylinks[$i]->arctype->real_path}/{$industrylinks[$i]->id}/</loc>
    <lastmod>".date('Y-m-d',strtotime($industrylinks[$i]->created_at))."</lastmod>
    <changefreq>daily</changefreq>
    <priority>1.0</priority>
</url>
            ";
            if ($i!=0 && $i%1000==0)
            {
                ...
                $urlsets='';
                $contents='';
            }elseif($i>(int)(1000*(ceil(count($industrylinks)/1000)-1))){
                $lastcontents=$urlsets;
            }
        }
        $contents=$mainsitemap.$urlset.$lastcontents.$urlset2;
...
";
    ...
    function MobileSitemap()
    {
        ...
        foreach ($urllinks as $urllink){
            $sitemapinfos.="<url> 
            <loc>$mappurl/news/{$urllink->id}/</loc> 
            <mobile:mobile type=\"mobile\"/>
            ...
            <url>
                <loc>$appurl/news/{$urllink->id}/</loc>
                <lastmod>".date('Y-m-d',strtotime($urllink->updated_at))."</lastmod>
                <changefreq>daily</changefreq>
                <priority>0.8</priority>
            </url>
