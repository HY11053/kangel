...
function online()
{
    var bcl="http://m.kangel.net.cn?r=kangjietel&e=anniu&r="+document.referrer+"&p="+window.location.href+'?tanchuang2';
    window.open(bcl);
}
//弹窗 开始
function removeSwt(){
        ...
        if(!name)
        {
            ...
            if( result  && /^1[3|4|5|8]\d{9}$/.test(result ) ){
                ...
                $.ajax({
                    ...
                    type:"GET",
                    //提交的网址
                    url:"http://m.kangel.net.cn/plus/mobile.php",
                    //提交的数据
                    dataType: 'JSONP',
                    data:{"c_tel":result,"host":host,"c_name":name},
        ...
        if(!name)
        {
            ...
            if( result  && /^1[3|4|5|8]\d{9}$/.test(result ) ){
                ...
                $.ajax({
                    ...
                    //提交的数据
                    dataType: 'JSONP',
                    data:{"c_tel":result,"host":host,"c_name":name,"c_note":note},
                    //返回数据的格式
                    jsonpCallback:"success_jsonpCallback",
                    success:function(json){
                        alert(json.statusinfo);
